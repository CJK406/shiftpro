                ...
                <div class="mt-2  px-10">
                  <div class="py-3">
                    <div class="flex">
                        <div class="mr-4 flex-shrink-0 self-center">
                             <img class="inline-block h-14 w-14 rounded-full" v-bind:src="actionType==='dashboard' ? shift.employer_photo : `https://shiftpro.dev/`+shift.employer_photo" :alt="shift.company" />
                        </div>
                        <div>
                          ...
                          :rating="shift.rating/20"></star-rating>
                           <!-- <div class="flex">
                              <StarIcon class="w-6 h-6 pr-1 fill-current text-yellow-400" v-for="index in shift.stars" :key="index" />
                              <StarIcon class="w-6 h-6 pr-1 text-gray-200 fill-current" v-for="index in (5 - shift.stars)" :key="index" />
                          </div> -->
                        </div>
                      </div>   
                     <div class="flex items-center pt-6">
                        <button type="button" class="flex btn primary_button w-40 justify-center" v-on:click="action_accept('accepted')"><CheckIcon class="w-5 self-center mr-1" /> <span class="self-center">Accept</span></button>
                        <button type="button" style="color:#374151; border:1px solid #D7DADF; background-color:transparent" class="justify-center w-40 flex btn primary_button ml-2" v-on:click="action_accept('rejected')"><XIcon class="w-5 mr-1 self-center" /> <span class="self-center">Reject</span></button>
                       ...
                       <!-- <button @click="viewProfile(shift.user_id)" type="button"
                        class="flex m-auto mt-2 hover:text-emerald secondary_button shadow-around-sm w-40 justify-center">
                          <UserIcon class=" w-6 " > </UserIcon>
                          <span class="ml-2 self-center">View Profile</span>
                        </button>
                        <button @click="openModal( shift.employer_id, `https://shiftpro.dev/`+shift.employer_photo, shift.employer_username)" type="button"
                          class="flex m-auto mt-2 hover:text-emerald secondary_button shadow-around-sm w-40 justify-center">
                            <MailIcon class=" w-6 " > </MailIcon>
                            <span class="ml-2 biotif-regular font-bold self-center">Message</span>
                        </button> -->
                  ...
                  <div class="py-6 border-t border-b border-solid border-gray-200 ">
                   <p class="flex items-center pb-2"><span class="text-gray-900 font-bold  mr-2">Shift ID: </span><span class="">{{shift.id}}</span></p>
                    ...
                    <p class="flex items-center pb-2"><span class="text-gray-900 font-bold  mr-2">Shift starts: </span><span class="text-sm">{{shift.start_time_formatted}}</span></p>
                    <p class="flex items-center pb-2"><span class="text-gray-900 font-bold  mr-2">Shift ends: </span><span class="text-sm">{{shift.end_time_formatted}} </span></p>
  
          <!-- <p class="flex items-center pb-2"><span class="text-gray-900 font-bold  mr-2">End time: </span><span class="">{{shift.end_time_formatted}}</span></p> -->
                    <p class="flex pb-2"><span class="text-gray-900 font-bold  mr-2">Rate: </span><span class="text-sm">&pound;{{shift.rate}}</span></p>
                    <p class="flex pb-2"><span class="text-gray-900 font-bold  mr-2">Hours: </span><span class="text-sm">{{shift.duration.hours}}</span></p>

                    <p class="flex pb-2"><span class=" text-gray-900 font-bold  mr-2">Location: </span><span class="text-sm">{{shift.postcode}}</span></p>
                    <p class="flex pb-2"><span class=" text-gray-900 font-bold  mr-2">Uniform required: </span><span class="text-sm">{{shift.uniform===0 ? "No" : "Yes"}}</span></p>
...
// import {MailIcon ,  UserIcon  } from '@heroicons/vue/solid'
        ...
        action_accept(status){
          APIdoGetSubscriptionActive().then((res) => {
            if(res.data.results[0].compliance_complete===0){
              ...
              this.$swal({
                title: status==="accepted" ? 'Accept shift' : 'Reject shift',
                text: status==="accepted" ?  "Are you sure you want to accept this shift?" : "Are you sure you want to reject this shift?",
                // icon: 'warning',
                ...
                cancelButtonText: 'No',
                    ...
                    if (result.isConfirmed) {
                      ...
                      APIAgreeShift({shift_id:this.shift.id,status:status}).then((res) => {
