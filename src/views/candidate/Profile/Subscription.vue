      ...
      <div class="lg:grid lg:grid-cols-4 gap-8">  
          <div class="lg:grid lg:col-span-3">
            <div class="lg:grid lg:grid-cols-2 gap-8 bg-white shadow-lg rounded-3xl">  
                <div class="p-4">
                    <div :class="{'subscription_active_image_div':user_detail.sub_status===1,'subscription_inactive_image_div':user_detail.sub_status!==1,}" class="lg:p-8 lg:pt-16 pt-16 p-4 pb-8 relative">
                       <img v-if="user_detail.sub_status===1" class="w-24 text-right absolute top-4 right-4" :src="require('../../../assets/sp-logo-w.png')" />
                        <img v-if="user_detail.sub_status!==1" class="w-24 text-right absolute top-4 right-4" :src="require('../../../assets/sp-logo.png')" />
                         <span 
                        :class="{'text-white':user_detail.sub_status===1,'text-blue-900':user_detail.sub_status!==1}"
                        class=" biotif-regular font-bold text-xl">{{user_detail.username}}</span>
                        <p 
                        :class="{'text-white':user_detail.sub_status===1,'text-blue-900':user_detail.sub_status!==1}"
                        class="text-sm font-semibold">Candidate subscription</p>
                ...
                <div>
                     <div class="relative mt-2  px-6 xl:px-10 py-4 xl:py-4  ">
                        <div>
                            <div class="grid lg:grid-cols-1 gap-8">
                                <div class="grid lg:grid-col-1">
                                    <div v-if="user_detail.sub_status===1" class="flex">
                                        <div>
                        ...
                        <button @click="changeSubscription" class="btn btn-full outline-none border-none bg-secondary hover:bg-secondary-light h-10 pb-2 lg:w-80 w-70 mt-8 lg:ml-auto m-auto biotif-regular">
                            {{user_detail.sub_status===1 ? 'Change my subscription type':'Make a payment'}}
                        </button>
                    </div>   
                </div>
            </div>
   ...
   computed: {
    ...
    methods: {
      ...mapActions({
       ...
       changeSubscription(){
           if(this.user_detail.sub_status===1){
               const data = {
                  isOpen: true,
                  iconName: 'TicketIcon',
