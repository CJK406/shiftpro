...
<script>
import Row from "./ShiftsAgreedTable/Row";
import RowMobile from "./ShiftsAgreedTable/Row_mobile";
import VPagination from "@hennge/vue3-pagination";
import "@hennge/vue3-pagination/dist/vue3-pagination.css";
import RowPanel from "./ShiftsAgreedTable/RowPanel";
import { mapGetters, mapActions } from "vuex";
import { mixin as VueClickAway } from "vue3-click-away";
import {
  APIgetEmployerShiftsAgreed,
...
export default {
    ...
    methods: {
        ...
        updateHandler() {
            ...
            for (let i = 0; i < this.shift_data_back.length; i++) {
                if(this.shift_data_back[i]["role"].includes(this.shift_filter)){
                    this.shift_data.push(this.shift_data_back[i]);
                }
            }
            setTimeout(() => {
                this.table = $("#datatable").DataTable({
                   "language": {
                      "emptyTable": "You currently have no agreed shifts",
                      paginate: {
                                previous: '&#x2190; Previous',
                                next: 'Next &#x2192;',
                            },
    ...
    mounted() {
        ...
        this.loader_flag=true;
        APIgetEmployerShiftsAgreed().then((res) => {
            this.loader_flag=false;

           this.shift_data = res.data.results;
           this.shift_data_back = res.data.results;
           this.updateHandler();
