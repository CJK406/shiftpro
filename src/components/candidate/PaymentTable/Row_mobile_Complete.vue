    ...
    <div class="p-4 mobile_td" >
      ...
      <div class="flex items-center">
          <div class="flex items-center top-1/2 left-2 transform -translate-y-1/2 lg:transform-none lg:static flex-shrink-0 h-10 w-10">
            <img @click="viewProfile(shift.employer_id)" class="h-8 w-8 rounded-full mt-10" :src="shift.photo" :alt="shift.photo" />
          </div>
          <div class="lg:ml-2">
            <div class="text-sm font-semibold text_username cursor-pointer">
              <span @click="viewProfile(shift.employer_id)">{{ shift.company }}</span>
            </div>
          </div>
        </div>
      <div class="flex text-main-light items-center mt-3">
        ...
        <button @click="submit" type="button" class="ml-auto focus:outline-none text-gray-400 biotif-regular font-bold text-emerald hover:text-secondary-light "
        >
        View
      </button>
      </div>
