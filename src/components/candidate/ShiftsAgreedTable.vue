                        ...
                        <div class="relative" v-click-away="onClickAway2">
                            <div class="flex items-center p-2 pl-4 mobile_td cursor-pointer" @click="open_sortbyDropdown">
                                <img 
                                    ...
                                    />
                                <span class="self-center text-sm ml-2 font-bold" style="color:rgb(116,117,118) ">SORT BY</span>
                            </div>
...
<script>
import Row from "./ShiftAgreeTable/Row";
import RowMobile from "./ShiftAgreeTable/Row_mobile";

import RowPanel from "./ShiftAgreeTable/RowPanel";
        ...
        updateHandler() {
            ...
            for (let i = 0; i < this.shift_data_backup.length; i++) {
                let miles = parseFloat(
                    this.shift_data_backup[i]["distance"].split(" miles")[0]
                );
                if (
                    parseFloat(this.distanceValue) >= miles &&
                    this.shift_data_backup[i]["role"].includes(
                        this.shift_filter
                    ) &&
                    (parseFloat(this.ratingValue.value[1]) >= parseFloat(this.shift_data_backup[i]["rate"])
                    && parseFloat(this.ratingValue.value[0]) <= parseFloat(this.shift_data_backup[i]["rate"])
                    )
                ) {
